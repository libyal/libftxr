     ...
     libcerror_error_t **error )
{
	...
	if( record_data_size < sizeof( ftxr_record_header_t ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	byte_stream_copy_to_uint16_little_endian(
	 ( (ftxr_record_header_t *) record_data )->record_type,
	 record_type );

	byte_stream_copy_to_uint32_little_endian(
	 ( (ftxr_record_header_t *) record_data )->record_size,
	...
	{
		byte_stream_copy_to_uint32_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown1,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint16_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown2,
		 value_16bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint64_little_endian(
		 ( (ftxr_record_header_t *) record_data )->file_identifier,
		 value_64bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint64_little_endian(
		 ( (ftxr_record_header_t *) record_data )->file_reference,
		 value_64bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown4,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown5,
		 value_32bit );
		libcnotify_printf(
		...
		if( libfguid_identifier_copy_from_byte_stream(
		     guid,
		     ( (ftxr_record_header_t *) record_data )->unknown6,
		     16,
		...
		byte_stream_copy_to_uint64_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown7,
		 value_64bit );
		libcnotify_printf(
		...
		if( libfdatetime_filetime_copy_from_byte_stream(
		     filetime,
		     ( (ftxr_record_header_t *) record_data )->unknown8,
		     8,
		...
		byte_stream_copy_to_uint32_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown9,
		 value_32bit );
		libcnotify_printf(
		...
		byte_stream_copy_to_uint64_little_endian(
		 ( (ftxr_record_header_t *) record_data )->unknown10,
		 value_64bit );
		libcnotify_printf(
...
#endif
	record_data_offset = sizeof( ftxr_record_header_t );

	if( ( (size_t) internal_record->size < sizeof( ftxr_record_header_t ) )
	...
	switch( record_type )
	{
		case LIBFTXR_RECORD_TYPE_UPDATE_JOURNAL_ENTRIES_LIST:
			if( libftxr_record_read_update_journal_entry_list(
			     internal_record,
			     record_data,
...
/* TODO refactor */
	if( record_type == LIBFTXR_RECORD_TYPE_UPDATE_JOURNAL_ENTRIES_LIST )
	{
		return( 1 );
	}
#if defined( HAVE_DEBUG_OUTPUT )
	if( libcnotify_verbose != 0 )
	{
		...
		libcnotify_print_data(
		 ...
		 (size_t) internal_record->size - sizeof( ftxr_record_header_t ),
		 0 );
	}
#endif
#if defined( HAVE_DEBUG_OUTPUT )
	 ...
	 || ( record_type == 0x07 ) )
	{
		...
		if( libcnotify_verbose != 0 )
		{
			...
			libcnotify_printf(
			 ...
			 function,
			 value_32bit );
			libftxr_debug_print_file_attribute_flags(
			 value_32bit );
			libcnotify_printf(
			 "\n" );
		}
     ...
     libcerror_error_t **error )
{
	...
	static char *function                     = "libftxr_record_read_update_journal_entry_list";
	size_t record_data_offset                 = 0;
	...
	if( record_data_size < ( sizeof( ftxr_record_header_t ) + 16 ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
